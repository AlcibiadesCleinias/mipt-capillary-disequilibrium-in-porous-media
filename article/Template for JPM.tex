...
\title{The double porosity model with fracture block matrices distribution of single-phase flow in a cracked porous medium}
...
\abstract{
In the article, we generalize the double porosity model of single-phase flow in a deformable medium with an elastic skeleton to the case of fracture block matrices distribution.
We consider the medium consists of low-permeable blocks (in the initial state) and a connected system of mesoscale fractures thereby the poroelastic and flow characteristics are rapidly oscillating functions.
With our model, we discover that the growth of small-scale fractures (scattered fracture) leads to an in the permeability of the matrix and the mass transfer intensification between distributed subsystems of double porosity.
By analyzing the inequality of dissipation, we derive the thermodynamically consistent governing relations and the equation for the evolution of the damage parameter in the matrix.
The final system of differential equations includes an equation of a medium equilibrium state, equations of pressures in the subsystems of double porosity, an equation for the scalar damage parameter, and consistent equations of dissipation inequality.
With the final system of the equations, we formulate an initial-boundary value problem that models a coupled flow processes, fracture processes, and changes in the stress-strain state of a double porosity layer (Initially the layer was in equilibrium and under high pore pressure).
}
...
\label{intro}
It is obvious, that interest in unconventional oil and gas development growths for the last time. Even more, and more petroleum companies and sometimes even governments face with the shale revolution [e.g. China https://energypolicy.columbia.edu/sites/default/files/China%20Shale%20Gas_WORKING%20DRAFT_Sept%2011_0.pdf].
